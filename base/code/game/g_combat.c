...
void player_die(gentity_t * self, gentity_t * inflictor, gentity_t * attacker, int damage, int meansOfDeath)
{
	...
	if(self->client->ps.pm_type == PM_DEAD || self->client->ps.pm_type == PM_WOUNDED)
	{
		return;
	}
...
//unlagged - backward reconciliation #2
	...
	{
		self->client->ps.pm_type = PM_WOUNDED;
	}
	else
	{
				  ...
				  const vec3_t dir, const vec3_t point, int damage, int dflags, int mod)
{
	...
	take = WeaveProtectDamage(targ, client, inflictor, attacker, dir, point, take, dflags, mod);
