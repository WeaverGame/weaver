...
void Cmd_Ready_f(gentity_t * ent)
{
	...
	if(ent->client->pers.ready || !GameIsInWarmup())
	{
		return;
	}
	...
	G_Printf("%s is ready.", ent->client->pers.netname);
}
...
void Cmd_Unready_f(gentity_t * ent)
{
	...
	G_Printf("%s is not ready.", ent->client->pers.netname);
}
...
=================
*/
void Cmd_ReadyTeam_f(gentity_t * ent)
{
	if(!ent || !ent->client)
	{
		return;
	...
	Team_ReadyPlayers(ent->client->sess.sessionTeam, qtrue);

	G_Printf("%s has readied the %s team!\n", ent->client->pers.netname, TeamName(ent->client->sess.sessionTeam));
}
...
=================
*/
void Cmd_UnreadyTeam_f(gentity_t * ent)
{
	if(!ent || !ent->client)
	{
		return;
	...
	Team_ReadyPlayers(ent->client->sess.sessionTeam, qfalse);

	G_Printf("%s has unreadied the % team!\n", ent->client->pers.netname, TeamName(ent->client->sess.sessionTeam));
}
