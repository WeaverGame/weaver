...
void ClientThreadEnd(gclient_t * client)
{
	...
	client->threading = qfalse;
	//CartesianToPolar(client->ps.stats[STAT_THREADX], client->ps.stats[STAT_THREADY], angle, distance);
	...
	{
		// add half a power div so the bottom of fire is on 0, and normalize
...
Based on ClientSpawn in g_client.c

Called when a wounded player's health is raised above 0.
Does some of the initialization in ClientSpawn, but without recreating the client struct.
Keeps position, etc.
============
*/
void ClientRevived(gentity_t *ent)
{
	...
	G_Printf("Client %d was wounded, now has health > 0.\n", client->ps.clientNum);
...
//      G_Printf("serverTime >>>>>\n" );
	...
	{
		if(client->ps.stats[STAT_HEALTH] > g_woundedHealth.integer)
		{
			client->ps.pm_type = PM_WOUNDED;
		}
		else
		{
	...
	else if(client->ps.pm_type == PM_WOUNDED)
	{
		// was wounded, now has health.
		ClientRevived(ent);
	}
	else if(client->grabHolder)
	{
	...
	{
		client->ps.speed *= WEAVE_SLOWPOISON_SLOWFACTOR;
	}
	if(client->ps.powerups[PW_HASTE])
	{
		client->ps.speed *= HASTE_SCALE;
	}
	if(client->ps.pm_flags & PMF_SPRINT)
	{
	...
	*/
	...
	if((client->ps.stats[STAT_HEALTH] < g_woundedHealth.integer) || (client->ps.pm_type == PM_DEAD) || (client->ps.pm_type == PM_WOUNDED && (client->ps.eFlags & EF_TAPOUT)))
	{
		...
		// is wounded but wounded isn't being used (should be in PM_DEAD)
...
void ClientEndFrame(gentity_t * ent)
{
	...
	if(ent->client->ps.powerups[PW_SLOWPOISONED])
	{
		...
		{
			G_Damage(ent, ent->client->slowAttacker, ent->client->slowAttacker, NULL, NULL, WEAVE_SLOWPOISON_DPS, 0,
	...
	if(ent->client->ps.powerups[PW_POISONED])
	{
		...
		{
			G_Damage(ent, ent->client->poisonAttacker, ent->client->poisonAttacker, NULL, NULL, ent->client->poisonDamage, 0,
	...
	for(i = 0; i < MAX_POWERUPS; i++)
	{
		if(ent->client->ps.powerups[i] < level.time)
		{
			...
			if(i == PW_SLOWPOISONED)
			{
				ent->client->slowAttacker = NULL;
			}
			if(i == PW_POISONED)
			{
				ent->client->poisonAttacker = NULL;
			}
		}
	}
