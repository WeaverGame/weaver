...
#define BUTTON7 128				//WEAVER weave a thread
#define BUTTON8 256				//WEAVER weave
...
void PM_Sprint(void)
{
	if(pm->cmd.buttons & BUTTON9 && (pm->cmd.forwardmove || pm->cmd.rightmove) && !(pm->ps->pm_flags & PMF_DUCKED))
	{
		if(pm->ps->powerups[PW_HASTE] > 0)
		{
			pm->ps->stats[STAT_STAMINA] += USE_STAMINA * pml.frametime;
			if(pm->ps->stats[STAT_STAMINA] > MAX_STAMINA)
			{
		...
		if(pm->ps->stats[STAT_STAMINA] > RECHARGE_CRIT_STAMINA)
		{
	...
	else
	{
		...
		pm->ps->stats[STAT_STAMINA] += RECHARGE_STAMINA * pml.frametime;
		if(pm->ps->stats[STAT_STAMINA] > CRIT_STAMINA)
		{
			pm->ps->stats[STAT_STAMINA] += RECHARGE_CRIT_STAMINA * pml.frametime;
		}
...
void PmoveSingle(pmove_t * pmove)
{
	...
	if(pm->ps->powerups[PW_SHIELDED] == 0)
	{
		// WEAVER
		// set weave a flag
		if(pm->cmd.buttons & BUTTON8)
		{
		...
		*/
		...
		// WEAVER
		// thread button pressed
		if(pm->cmd.buttons & BUTTON7)
		{
	...
	if(pm->ps->pm_type == PM_FREEZE)
	{
		...
		pm->cmd.rightmove = 0;
		pm->cmd.upmove = 0;
		PM_Weapon();			// can still cast and stuff
		//PM_ClipVelocity(pm->ps->velocity, pml.groundTrace.plane.normal, pm->ps->velocity, OVERCLIP);
		PM_CheckDuck();			// adds mins and maxs for clipping
		PM_SlideMove(qfalse);
