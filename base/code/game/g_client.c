...
void ClientBegin(int clientNum)
{
	...
	{
		tent = G_TempEntity(ent->client->ps.origin, EV_SPAWNTIME);
		tent->r.svFlags |= SVF_SINGLECLIENT;
		tent->r.singleClient = ent->s.number;
		if(client->sess.sessionTeam == TEAM_RED)
		{
			tent->s.time2 = level.teamSpawnPreviousTimeRed;
		}
		else
		{
			tent->s.time2 = level.teamSpawnPreviousTimeBlue;
		}
	}
...
//  client->areabits = savedAreaBits;
	...
	*/
	...
	else
	{
		...
		//Weaver do not default to a weapon.
		/*
