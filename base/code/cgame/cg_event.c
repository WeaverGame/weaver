...
//unlagged - attack prediction #2
		...
		case EV_SPAWNTIME:
			...
			cg.teamSpawnPrevious = cent->currentState.time2;
			s = CG_ConfigString(CS_SERVERINFO);
			if(cgs.clientinfo[cg.clientNum].team == TEAM_RED)
			{
				cg.teamSpawnPeriod = atoi(Info_ValueForKey(s, "g_teamSpawnRed")) * 1000;
			}
			else if(cgs.clientinfo[cg.clientNum].team == TEAM_BLUE)
			{
				cg.teamSpawnPeriod = atoi(Info_ValueForKey(s, "g_teamSpawnBlue")) * 1000;
			}
			break;
		...
		case EV_WEAVEMISSILE_HIT:
			...
			ByteToDir(es->eventParm, dir);
			CG_WeaveMissileHitPlayer(es->weapon, position, dir, es->otherEntityNum);
			break;
